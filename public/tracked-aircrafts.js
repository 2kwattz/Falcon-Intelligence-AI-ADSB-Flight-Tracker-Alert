(() => {
  ...
  function showSatelliteMap(latitude, longitude, label, heading) {
    const container = $("#aircraft-location-map");
    if (!container || !window.L) return;
    if (detailMap) detailMap.remove();
    detailMap = window.L.map(container, { zoomControl: true, attributionControl: true }).setView([latitude, longitude], 10);
    window.L.tileLayer("https://tiles.maps.eox.at/wmts/1.0.0/s2cloudless-2024_3857/default/g/{z}/{y}/{x}.jpg", {
      minZoom: 2,
      maxZoom: 13,
      attribution: '&copy; <a href="https://s2maps.eu/" target="_blank" rel="noreferrer">Sentinel-2 cloudless</a> by EOX (Contains modified Copernicus Sentinel data)'
    }).addTo(detailMap);
    detailMap.createPane("location-labels");
    detailMap.getPane("location-labels").style.zIndex = 550;
    detailMap.getPane("location-labels").style.pointerEvents = "none";
    window.L.tileLayer("https://tiles.maps.eox.at/wmts/1.0.0/overlay_3857/default/g/{z}/{y}/{x}.jpg", {
      minZoom: 2,
      maxZoom: 18,
      pane: "location-labels",
      attribution: '&copy; <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OpenStreetMap contributors</a>; overlay rendered by EOX'
    }).addTo(detailMap);
    const bearing = Number.isFinite(heading) ? heading : 0;
    const icon = window.L.divIcon({ className: "aircraft-map-marker", html: `<span class="aircraft-map-marker__inner" aria-hidden="true"><svg viewBox="0 0 24 24" style="--bearing:${bearing}deg"><path d="M12 2 15.2 9.4l5.3 2.1v2l-5.3-.55V18l2.1 2v1H6.7v-1l2.1-2v-5.05l-5.3.55v-2l5.3-2.1L12 2Z"/></svg></span>`, iconSize: [40, 40], iconAnchor: [20, 20] });
    window.L.marker([latitude, longitude], { icon, title: label }).addTo(detailMap).bindTooltip(label, { direction: "top", offset: [0, -16] }).openTooltip();
